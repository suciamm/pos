<?php 
	$product =	ucfirst($_GET['product']);

	$db  	= model('M_product');
	$fetch 	= $db->view_product($product);
	$data 	= $fetch['data'];
	$row 	= $fetch['row'];
	...
	<div class="row pt-3">
		<div class="row col-md-8 col-sm-12">
			<ul class="breadcrumb action-bar">				
				<li class="mr-4">
					<a href="#" id="add" data-id='index.php?page=product&sub=add&product=<?php echo $product;?>'>
		...
		<div class="col-md-4 col-sm-12">
			...
			<li class="breadcrumb-item active"><b><?php echo $product; ?></b></li>
			</ul>
		</div>
	</div>	
</div>
					...
					<table class="table table-hover table-striped" id="view-table">
						...
						<tbody>
							...
							<tr>
								<td>
									<div class="i-checks">
									<input type="checkbox" class="check checkbox-template" name="id[]" value="<?php echo $view['image']; ?>">
									</div>
								</td>
								<td><?php echo $no++; ?></td>
								<td><?php echo $view['product_code']; ?></td>								
								<td>
									<div class="image">
										<img src="image/upload/<?php echo $view['image']; ?>" alt="..." class="img-fluid rounded-circle" width="30">
										<?php 
										echo $view['product'];
										?>
									</div>
								...
								<td>Rp <?php echo rupiah($view['price']); ?></td>	
								<td>
									<?php 
										if($view['status'] == 'ready') {
											...
											echo '<span class="badge badge-danger">'.$view['status'];'</span>';
										}
									?>										
								</td>	
								<td>
									<a href="index.php?page=product&sub=edit&product=<?php echo $product;?>&id=<?php echo base64_encode($view['id']);  ?>">
									<i class="fa fa-pencil mr-1"></i>
									</a>
									
									<a href="#" data-toggle="modal" data-target="#btn_delete" data-id="<?php echo $view['image']; ?>">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    ...
	    //Pencarian//
	    $('.showBystatus').change(function(event) {
	    	var data = $(this).val();
	    	if(data == '') {
	    		window.filter_column(7,'');
	    	}
	    	else {
	    		window.filter_column(7,data);
	    	}
	    });
