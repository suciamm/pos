	...
	<div class="col-lg-8 col-sm-12">
		...
		<div class="row m1-2">
			<div class="row col-lg-8 col-sm-12 mt-3">
				<span class="col-lg-12 ml-2">
					<h4>ORDER PRODUCT <label class="label-menu"></label></h4>
				</span>
			</div>
			<div class="col-lg-4 col-sm-12 material-search">
				<ul class="row">
					<span class="col-10 mt-1">
						<input type="text" name="" class="input-material none autoList" placeholder="Search Product .."
							id="search-product">
					</span>
				...
				<ul class="row">
					<?php foreach ($getMenu['data'] as $menu) { ?>
						<li class="card menu-product cbig text-center cat-menu" data-value="<?php echo $menu['menu']; ?>">
							<img src="image/upload/<?php echo $menu['icon']; ?>" alt="..." class="img-responsive mb-3"
								width="100">
			...
			<div class="col-12 d-none" id="payment-bill">
				<div class="row pl-3 pr-3 pt-1">
					...
					<div class="col-6 text-right p-0">
						<span class="col-12 fs-11 p-0 pointer detail-payment" data-toggle="modal"
							data-target="#payment-detail-modal" data-id="">
							detail
				...
				<div class="row pl-3 pr-3 mt-3 d-none" id="payment-method-noncash">
					...
					<div class="col-12 p-0" id="payment-noncash-detail">
						...
						<div class="row">
							...
							<span class="col-6 text-right text-bold" id="payment-noncash-tax"></span>
						</div>
						<div class="row">
							<span class="col-6">Service Charge</span>
							<span class="col-6 text-right text-bold" id="payment-noncash-service"></span>
						</div>
						<div class="row">
		...
		<div class="row card">
			<div class="col-12 pt-3 pb-3">
				<div class="row b-bottom pb-3">
					...
					<span class="col-6 text-right pointer" id="close-bill-order-change"><i
								...
								<div class="row p-2 bg-material">
									<div class="col-8">
										<div class="row text-center pb-1">
											<span class="col-4 p-0 pr-1">
												<span class="col-12 m-0 p-3 card pointer payment-input-number"
													data-id="7">7</span>
											</span>
										...
										<div class="row text-center pb-1">
											...
											<span class="col-4 p-0 m-0 pt-1">
												<span class="col-12 m-0 p-3 card pointer payment-input-number"
													data-id="x"><i class="fa fa-close pt-1 pb-1"></i></span>
											</span>
										</div>
									</div>
									...
									<div class="col-4">
										<div class="row text-center pl-1 pb-1">
											<span class="col-12 p-0">
												<span class="col-12 m-0 p-3 card pointer payment-input-number"
													data-id="clear">CLEAR</span>
											</span>
										</div>
										<div class="row text-center pl-1 pb-1">
											<span class="col-12 p-0">
												<span
													class="col-12 m-0 p-3 card pointer payment-input-number btn-primary text-white"
													data-id="print"">PRINT</span>
												</span>
											</div>
											<div class=" row text-center pl-1 pb-1">
													<span class="col-12 p-0">
														<span
															class="col-12 m-0 pt-4 pr-5 pb-5 pl-5 card pointer payment-input-number"
															data-id="enter">
									...
									<div class="col-12">
										<div class="row text-center">
											<span class="col-12 p-0">
												<span class="col-12 m-0 p-3 card payment-input-number btn-primary"
													data-id="finish">FINISH</span>
											</span>
										</div>
									</div>
								</div>
...
<script type="text/javascript">
	...
	function allProduct(url) {
		$.ajax({
			url: url,                // URL endpoint untuk permintaan AJAX
			type: 'POST',            // Tipe HTTP request, dalam hal ini POST
			data: { 'aksi': 'getAllProduct' }, // Data yang dikirim ke server, dalam hal ini objek dengan key 'aksi' dan value 'getAllProduct'
			dataType: 'html',        // Tipe data yang diharapkan dari server, dalam hal ini HTML
			success: function (response) { // Fungsi yang dijalankan ketika permintaan berhasil
				var data = $.parseJSON(response); // Parsing respons JSON dari server
				$('#list-product').html(data.output); // Memperbarui konten elemen dengan id 'list-product' menggunakan data yang diterima
				$('#pages').val(data.pages); // Mengatur nilai elemen dengan id 'pages' menggunakan data yang diterima
			}
		});
	}
	...
	function disableButton(id1, id2) {
		// Mengubah properti CSS dari elemen-elemen dengan id yang diberikan
		$(id1, id2).css({
			'disabled': true,          // Menonaktifkan elemen
			'cursor': 'not-allowed',   // Mengubah kursor menjadi 'not-allowed'
			'background': '#444958'    // Mengubah warna latar belakang
		});
	}
	...
	function addRow(image, product, price, product_code) {
		// Membuat nama kelas untuk baris baru berdasarkan nama produk
		var tr_class = product.replace(/\s+/g, '').toLowerCase();
		// Membuat markup baris baru dengan data yang diberikan
		var row =
			'<tr class="' + tr_class + '">' +
			'<td class="w-60">' +
			'<div class="row">' +
			...
			'</td>' +
			'</tr>';
		return row; // Mengembalikan markup baris baru
	}



	function listTable(url) {
		// Mengirim permintaan AJAX untuk mendapatkan daftar tabel
		$.ajax({
			url: url,                      // URL endpoint untuk permintaan AJAX
			type: 'POST',                  // Tipe HTTP request, dalam hal ini POST
			data: { 'aksi': 'listTable' },  // Data yang dikirim ke server, dalam hal ini objek dengan key 'aksi' dan value 'listTable'
			dataType: 'html',              // Tipe data yang diharapkan dari server, dalam hal ini HTML
			success: function (response) {  // Fungsi yang dijalankan ketika permintaan berhasil
				var data = $.parseJSON(response); // Parsing respons JSON dari server
				$('#listTable-modal').html(data.output); // Memperbarui konten modal dengan data yang diterima
			}
		});
	}
	...
	// Contoh cara memanggil fungsi setelah pengguna memilih metode pembayaran
	

// function paymentNonCash(sales_code, id) {
//     $.ajax({
//         url: 'url',
//         type: 'POST',
//         data: { 'aksi': 'paymentNonCash', 'sales_code': sales_code, 'id_payment': id },
...
//                 $('#payment-noncash-grandtotal').text(data.grand_total);
//                 $('#payment-noncash-discount').text(data.discount);
//                 $('#payment-noncash-tax').text(data.tax); // Mengupdate nilai tax
//                 $('#payment-noncash-service').text(data.service_charge); // Mengupdate nilai service charge
//                 $('#payment-noncash-total').text(data.total);
//             }
//             else {
...
//                 $('#payment-noncash-grandtotal').text('');
//                 $('#payment-noncash-discount').text('');
//                 $('#payment-noncash-tax').text(''); // Menghapus nilai tax jika gagal
//                 $('#payment-noncash-service').text(''); // Menghapus nilai service charge jika gagal
//                 $('#payment-noncash-total').text('');
//             }
//         },
...
function paymentNonCash(url, sales_code, id) {
    $.ajax({
        ...
        type: 'POST',
        data: { 'aksi': 'paymentNonCash', 'sales_code': sales_code, 'id_payment': id },
        dataType: 'json', // Menggunakan JSON langsung
        success: function (data) {
            ...
            if (data.status === 'success') {
                ...
                $('#payment-noncash-discount').text(data.discount);
                $('#payment-noncash-tax').text(data.tax); // Mengupdate nilai tax
                $('#payment-noncash-service').text(data.service); // Mengupdate nilai service charge
                $('#payment-noncash-total').text(data.total);
            } else {
                console.error('Error:', data.msg);
                $('#payment-noncash-item').text('');
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            console.error('AJAX error:', textStatus, errorThrown);
            console.error('Response Text:', jqXHR.responseText); // Debugging error server
        }
    }); 
}
...
function paymentCash(url, sales_code, id) {
    console.log('Sales Code:', sales_code);
	...
	$(document).ready(function () {
		...
		var tax = toInteger($('.price_tax').text());
			...
			if (getCookie('menu') == 'menu-order') {
				...
				$.ajax({
					...
					success: function(response) {
						...
						var dsc = response.hasil["discount"];		
						if (dsc) {
					...
					error: function(xhr, status, error) {
						console.error("AJAX error: " + status + ' - ' + error); // Tambahkan penanganan kesalahan
					}
				});
				...
				$.ajax({
					...
					success: function(response) {
						console.log(response); // Tambahkan log untuk debugging
						var svg = response.hasil["persentage"];		
						// console.log()
						if (svg) {
							var harga_service = priceInt * (svg / 100);
				...
				if (tr < 1) {
					...
					if (same_product[0] == product) {
						...
						var gPrice_tax = toInteger($('.price_tax').text());
						var gPrice_service = toInteger($('.price_service').text());
						...
						var subtotal_item = $('.' + tr_class).find('.subtotal_item').val(rupiah(subtotal_item));
						var subtotal = $('.price_subtotal').text(rupiah(subtotal));
					...
					if (getCookie('open') == 'menu-order-change') {
						...
						if (tr < 1) {
							...
							if (same_product[0] == product) {
								...
								var gPrice_tax = toInteger($('.price_tax-change').text());
								var gPrice_service = toInteger($('.price_service-change').text());
		...
		$('.table-order').on('click', '.minus', function (event) {
			...
			var gTotal_item = toInteger($('.total_item').text());
			var gPrice_subtotal = toInteger($('.price_subtotal').text());
			var gPrice_discount = toInteger($('.price_discount').text());
			var gPrice_tax = toInteger($('.price_tax').text());
			var gPrice_service = toInteger($('.price_service').text());
			...
			var total = subtotal + gPrice_tax + gPrice_service - gPrice_discount;
				...
				$.ajax({
					...
					async: false,
					success: function (response) {
						var disc = response.hasil["discount"];
						if (disc) {
			...
			if (gQty_item > 1) {
				...
				$('.total_item').text(gTotal_item - 1);
				$('.price_subtotal').text(rupiah(subtotal));
				// $('.price_discount').text(rupiah(gPrice_discount));
				$('.price_total').text(rupiah(total));
				$('.hide-total').val(total);
			}
		});
		...
		$('.table-order').on('click', '.remove-item', function (event) {
			...
			$('.total_item').text(gTotal_item - gQty_item);
			$('.price_subtotal').text(rupiah(subtotal));
			$('.price_total').text(rupiah(total));
			$('.hide-total').val(total);
